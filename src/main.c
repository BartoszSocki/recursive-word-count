...
int main(int argc, char** argv) {
	...
	Patterns* blacklist = patterns_init();

	while ((opt = getopt(argc, argv, ":d:w:b:")) != -1) {
		switch (opt) {
			case 'd':
				path = optarg;
				break;
			case 'w':
				...
				break;
			case 'b':
				patterns_add_pattern(blacklist, optarg);
				break;
			default:
				break;
		}
	...
	while ((node = fts_read(fs)) != NULL) {
		...
		if (blacklist->patterns && patterns_is_match(blacklist, node->fts_path))
			continue;
